...
public class Cronus implements Aspect {
    @Override
    ...
    public AbilityResult cleaveAbility(EntityDamageByEntityEvent event) {
        if (event.getDamager() instanceof Player) {
            ...
            if (DGGame.PLAYER_R.fromPlayer(player).getMajorDeity().equals(Aspects.CRONUS)) {
    ...
    public AbilityResult timeControlAbility(PlayerInteractEvent event) {
        ...
        for (long i = worldTime; i <= newTime; i += 1000) {
            count++;
            final long finalI = i;
            Bukkit.getScheduler().scheduleSyncDelayedTask(DGGame.getInst(), new Runnable() {
                @Override
                public void run() {
